      ...
      <q-card class="chat-panel flex flex-col no-wrap bg-white border-l border-zinc-200 shadow-2xl">
        ...
        <q-card-section
          ...
          <div class="space-y-0.5">
            <div class="text-sm font-black tracking-tight text-zinc-900">
              Assistant IA</div>
            <div class="text-[11px] text-zinc-500 font-medium">Recherche & Concierge automatisé</div>
          </div>
          <div class="flex items-center gap-1.5">
            <q-btn flat round dense class="text-zinc-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200"
              @click="clearChat" title="Effacer l'historique des conversations">
        ...
        <q-card-section ref="messagesContainer" class="chat-messages flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-50">
          <div v-if="messages.length === 0" class="flex flex-col items-center justify-center py-12 space-y-3">
            ...
            <p class="text-sm font-bold text-zinc-800">Bienvenue sur TakeLog Chat</p>
                  ...
                  <router-link v-for="item in msg.widgetData" :key="item.id" :to="`/logements/${item.id}`"
                    ...
                    <div
                      ...
                      <div class="p-3 space-y-1.5">
                        ...
                        <div class="text-[10px] text-zinc-500 font-medium">{{ item.ville }} · {{ item.nb_places }}
                          places
                        </div>
        ...
        <q-card-section class="p-3.5 border-t border-zinc-200 bg-white sticky bottom-0">
          <q-form @submit.prevent="sendMessage" class="flex items-center gap-2.5">
            <div class="flex-1 relative">
              <q-input v-model="draft" outlined dense placeholder="Ex: studio à Kairouan max 400DT..." maxlength="400"
                ...
                :disable="sending" @keydown.enter.exact.prevent="sendMessage" />
            </div>
...
import { MessageSquare, X, Send, ChevronDown, Loader2, Database, Brain, CheckCircle, Trash2 } from 'lucide-vue-next'
...
async function clearChat() {
  await chatStore.clearConversation(props.userId, props.agentId)
}
    ...
    for await (const payload of result.stream) {
      ...
      switch (payload.type) {
        ...
        case 'tool_result': {
          ...
          // Check if this is POI data (has latitude/longitude)
          if (data && data.data && Array.isArray(data.data)) {
            // POI search result - show on map
            ...
            // Add user's current location as first point if available
            ...
            bot.mapData = { points }
            bot.toolStatus = `${count} point(s) d'intérêt trouvé(s)`
          }
          // Check if this is logement data
